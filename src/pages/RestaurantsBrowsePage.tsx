...
import { Search, SlidersHorizontal, MapPin, UtensilsCrossed, ChevronRight, Navigation, Heart, List, Map as MapIcon } from 'lucide-react'
...
export function RestaurantsBrowsePage({ onNavigate, onAuthClick }: RestaurantsBrowsePageProps) {
  ...
  const { restaurants: allRestaurants, loading, error, locationNotFound } = useRestaurants({ search, cuisine: cuisines, radiusKm, ensureCoordinates: view === 'map' })
  const { isFavorite, toggleFavorite } = useFavorites()
  const restaurants = onlyFavorites ? allRestaurants.filter(r => isFavorite(r.id)) : allRestaurants
  const hasFilters = search || cuisines.length > 0 || radiusKm || onlyFavorites
    ...
    <div className="min-h-screen bg-gray-50 dark:bg-gray-900">
      ...
      <main className="max-w-5xl mx-auto px-4 py-6 pb-24 md:pb-8">
          ...
          <div className="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 p-4 mb-6 animate-fade-in space-y-5">
            <div>
              <div className="flex items-center justify-between mb-3">
                <h3 className="font-medium text-gray-900 dark:text-white">Tipo de cocina <span className="text-gray-400 dark:text-gray-500 font-normal">(elige tantos como quieras)</span></h3>
                {hasFilters && (
                  <button onClick={() => { setSearch(''); setCuisines([]); setRadiusIndex(0); setOnlyFavorites(false) }} className="text-xs text-primary-600 font-medium">
                    Limpiar filtros
                  </button>
                )}
              </div>
            ...
            <div className="pt-4 border-t border-gray-100 dark:border-gray-700">
              <div className="flex items-center gap-2 mb-1">
                ...
                <h3 className="font-medium text-gray-900 dark:text-white">Radio de búsqueda</h3>
              </div>
        ...
        {loading ? (
          ...
          <>
            <p className="text-sm text-gray-500 dark:text-gray-400 mb-4">{restaurants.length} restaurantes{radiusKm ? ` a menos de ${radiusKm} km` : ''}</p>
            {view === 'map' ? (
